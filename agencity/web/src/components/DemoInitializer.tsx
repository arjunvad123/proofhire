...
/**
 * DemoInitializer - Automatically sets up Confido demo environment
 * if no onboarding state exists in localStorage
 */
export function DemoInitializer() {
  useEffect(() => {
    ...
    const existing = localStorage.getItem('onboarding-state');
    if (existing) {
      console.log('✅ Demo state already initialized');
      return;
    }

    // Initialize with Confido demo data
    const confidoData = {
    ...
    console.log("🚀 Demo environment auto-initialized for Confido!");
    console.log("   Company: Confido");
