"""
Test script for organization profile management -- REST API Edition.

Demonstrates creating and managing organization profiles using the new Supabase REST service.
"""
...
async def test_profile_crud():
    """Test CRUD operations for organization profiles via REST API."""
    logger.info("=" * 60)
    logger.info("Testing Organization Profile Management (REST API)")
    logger.info("=" * 60)

    # Use a random workspace ID to avoid conflicts
    test_workspace_id = f"T_TEST_{uuid.uuid4().hex[:8]}"
    ...
    logger.info(f"  - Onboarding complete: {profile.get('onboarding_complete')}")

    # Test 2: Get existing profile
    logger.info("\n2. Getting existing profile...")
    existing = await org_profiles_db.get_or_create_profile(
        slack_workspace_id=test_workspace_id,
        slack_workspace_name="Test Workspace",
    )
    ...
    updated = await org_profiles_db.update_profile(
        profile['id'],
    ...
    knowledge1 = await org_profiles_db.add_knowledge(
    ...
    knowledge2 = await org_profiles_db.add_knowledge(
        org_profile_id=profile['id'],
        category="red_flag",
    ...
    all_knowledge = await org_profiles_db.get_knowledge(profile['id'])
    ...
    imported = await org_profiles_db.import_onboarding_data(
    ...
    success_profile = await org_profiles_db.mark_onboarding_complete(profile['id'])
    logger.info(f"✓ Onboarding complete: {success_profile['onboarding_complete']}")

    # Final verification
    logger.info("\n8. Final profile state...")
    final = await org_profiles_db.get_profile_by_slack_workspace(test_workspace_id)
